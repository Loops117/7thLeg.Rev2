...
const DEFAULT_DESKTOP_HINT = `Drag elements on the stage. Layout is rendered on a fixed ${THEATRICAL_STAGE_REF_WIDTH_PX}px-wide canvas and scaled to fit — the same positions and text wrapping you see here appear on the storefront (text stays selectable, not an image). Upload an .mp4 or .webm, or paste a direct video file URL — no YouTube/Vimeo player chrome.`;

export type TheatricalPaneEditorPanelProps = {
  ...
  /** Optional heading above controls (mobile editor). */
  sectionTitle?: string;
  /** Override the helper text under the heading. */
  hint?: string;
  /** Constrain preview width to simulate a phone viewport (px). */
  previewMaxWidthPx?: number;
};

export function TheatricalPaneEditorPanel({
  stageAspect,
  stageMaxHeightPx,
  stageBgHex,
  elements,
  ...
  return (
    <div className="theatrical-pane-editor space-y-4">
      {sectionTitle ? <h3 className="text-sm font-black text-palm">{sectionTitle}</h3> : null}
      <p className="theatrical-pane-editor__hint text-xs">{hint}</p>
      ...
      <div
        className={previewMaxWidthPx ? "mx-auto w-full" : undefined}
        style={previewMaxWidthPx ? { maxWidth: previewMaxWidthPx } : undefined}
      >
